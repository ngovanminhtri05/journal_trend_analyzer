...
| 8.3 | Report Export: build dashboard PDF (`pdf`) → upload Firebase Storage → show URL `[tdd:skip:ui]` | PDF uploads; download URL shown | 0.3, 4.1 | cc:done — pure `buildDashboardReportPdf` (ASCII-safe) + `StorageService`/`ReportStorageApi`; `HomeViewModel.exportReport(uid)` builds→uploads to `reports/{uid}/…`→shows URL dialog (copy link)→logs `export_pdf`. Unit-tested (builder + export happy/no-op/error). **Enable Storage in console.** |
